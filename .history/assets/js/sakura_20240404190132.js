...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
        ...
        var fontSize = 36;
        var font = fontSize + 'px Arial'; // 设置字体样式
    
        ctx.font = font;
        ctx.fillStyle = 'black'; // 设置文字颜色
    
        var index = 0; // 用于记录当前绘制到的文字索引
    
        var timer = setInterval(function() {
            ctx.clearRect(0, 0, canvas.width, canvas.height); // 清空画布
    
            // 绘制已经显示的文字
            ctx.fillText(text.substr(0, index), 50, 50);
            ...
            if (index >= text.length) {
                clearInterval(timer); // 文字全部显示完毕，清除计时器
            }
        }, 500); // 设置文字逐渐显示的速度（每隔500毫秒绘制一个文字）
    }, 9999);
