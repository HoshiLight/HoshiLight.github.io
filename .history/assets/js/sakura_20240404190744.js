...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.head.appendChild(style);
     ...
     // 获取所有的文字元素
     var chars = document.querySelectorAll('#text-container span');
     // 定义逐个显示文字的动画函数
     function showChars() {
         chars.forEach(function(char, index) {
             setTimeout(function() {
                 char.style.display = 'inline'; // 显示文字
                 char.style.animation = 'fadeIn 0.5s ease-out'; // 添加动画效果
             }, index * 200); // 设置每个文字之间的延迟时间
         });
     }
     // 调用函数开始逐个显示文字
     showChars();
